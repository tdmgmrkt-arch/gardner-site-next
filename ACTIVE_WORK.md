...
- **Rebrand announcement — "Gardner Plumbing Company is becoming GPC Home Pros" — PUSHED to branch `feat/gpc-rebrand-announcement` (2026-09-15, commit `6d36cba`). NOT merged; `main` and production untouched.** PR to be opened manually at https://github.com/tdmgmrkt-arch/gardner-site-next/pull/new/feat/gpc-rebrand-announcement (`gh` CLI is not installed on this machine). **Scope decision: this is a HEADS-UP that the name change is coming, not an announcement that it happened** — no GBP rename, no citation sweep, no domain change, and the site-wide schema `name` deliberately still reads "Gardner Plumbing Co." Only the popup + its landing page ship.
  - **Only 7 files were staged.** The repo had 30 other uncommitted entries (blog batch, HVAC page edits, `deliverables/`, images) that must NOT ride along on this push — they are still uncommitted and still pending.
  - **Mobile bar is an OVERLAY, not in-flow (final iteration).** Kenneth: *"i dont want it to push everything else down... just have everything else how it was before, below it."* First attempt floated it at the very top of the page, which covered the logo, hamburger menu, and emergency phone number — a first impression with no navigation. Fixed by wrapping `<main>` in a `relative` div in `app/layout.tsx` and anchoring the bar `absolute inset-x-0 top-0` inside it, so it floats over the hero photo while all header chrome stays visible and tappable. Verified header top 23px and hero H1 top 225px, both identical to baseline — nothing moved.
  - **CLS regression found and fixed before push.** Homepage baseline CLS is **0.000**; the original in-flow bar inserting after hydration took it to **0.0957**, a hair under the 0.1 failing threshold and likely over it on a real mid-range phone. Fixed by server-rendering the bar plus a blocking pre-paint script in `<head>` that sets `.gpc-notice-seen` on `<html>` for dismissed visitors (CSS hides it, no shift). That fix caused a React hydration mismatch, resolved with `suppressHydrationWarning` on `<html>`. Final: **0.000 on first visit and on return visit.** The overlay approach makes this doubly safe since an out-of-flow element cannot shift layout at all.
  - **Original (superseded) description of the in-flow build:** Rebrand was already live externally (Instagram display name + third-party listings) while the site still said Gardner Plumbing Co. only. Shipped locally:
